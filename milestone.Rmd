...
Here I've read just a sample of each file. Each sample contains 100 lines from the original.
...
The first cleaning step is to get rid of the profanity words. For that, I used the [What Do You Love](http://www.wdyl.com/profanity?q=) webservice from Google to discover what words are profane:
...
To build the corpora it remains necessary to remove punctuation, numbers, profanity, stop words and strip any whitespace. As a normalization step, I've lowered cases for all words.
...
Here's three wordclouds I build for each one of the n-grams calculated.
...
```{r warning=FALSE}
wordcloud(bigram$Term,bigram$Freq, scale=c(8,.2),min.freq=1, max.words=Inf, random.order=FALSE, rot.per=.15, colors=pal2)
```

```{r warning=FALSE}
wordcloud(threegram$Term,threegram$Freq, scale=c(8,.2),min.freq=1, max.words=Inf, random.order=FALSE, rot.per=.15, colors=pal2)
```
...
* the user, if sees his/her word, clicks on it, it appears on the textbox and the user keeps on writing.
